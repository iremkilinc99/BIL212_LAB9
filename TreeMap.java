...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
    ...
    protected static class BalanceableBinaryTree<K, V>
        ...
        protected Position<Entry<K, V>> rotateR(Position<Entry<K, V>> p) {
            BSTNode<Entry<K, V>> z = (BSTNode<Entry<K, V>>) validate(p);
            BSTNode<Entry<K, V>> y = (BSTNode<Entry<K, V>>) z.getLeft();
            BSTNode<Entry<K, V>> x = (BSTNode<Entry<K, V>>) z.getParent();
        ...
        protected Position<Entry<K, V>> rotateL(Position<Entry<K, V>> p) {
            BSTNode<Entry<K, V>> z = (BSTNode<Entry<K, V>>) validate(p);
            BSTNode<Entry<K, V>> y = (BSTNode<Entry<K, V>>) z.getRight();
            BSTNode<Entry<K, V>> x = (BSTNode<Entry<K, V>>) z.getParent();
        ...
        protected Position<Entry<K, V>> rotateLR(Position<Entry<K, V>> p) {
            BSTNode<Entry<K, V>> z = (BSTNode<Entry<K, V>>) validate(p);
            BSTNode<Entry<K, V>> x = (BSTNode<Entry<K, V>>) z.getParent();
            BSTNode<Entry<K, V>> w = (BSTNode<Entry<K, V>>) z.getLeft();
            rotateL(w);
            return rotateR(z);
        }

        protected Position<Entry<K, V>> rotateRL(Position<Entry<K, V>> p) {
            BSTNode<Entry<K, V>> z = (BSTNode<Entry<K, V>>) validate(p);
            BSTNode<Entry<K, V>> x = (BSTNode<Entry<K, V>>) z.getParent();
            BSTNode<Entry<K, V>> w = (BSTNode<Entry<K, V>>) z.getRight();
            rotateR(w);
            return rotateL(z);
        }
     ...
     */
    protected ArrayList<Position<Entry<K, V>>> positions(Position<Entry<K, V>> localRoot) {
        ArrayList<Position<Entry<K, V>>> arr = new ArrayList<>();
        for (Position<Entry<K, V>> p : tree.preorder(localRoot)) {
            if (p != null)
                arr.add(p);
        }
        return arr;
    ...
    public V put(K key, V value) throws IllegalArgumentException {
        ...
        if (isExternal(p)) {                    // key is new
            ...
            /*((BalanceableBinaryTree.BSTNode<Entry<K,V>>)p).setAux(tree.height(p));
            while(tree.parent(p)!=null&&size()!=1){
                ((BalanceableBinaryTree.BSTNode<Entry<K,V>>)p).setAux(tree.height(p));
                p=tree.parent(p);
            }
            ((BalanceableBinaryTree.BSTNode<Entry<K,V>>)tree.root).setAux(tree.height(tree.root)); //for root
             */
    ...
    // additi onal behaviors of the SortedMap interface
    ...
    public Iterable<Entry<K, V>> entrySet() {
        ...
        for (Position<Entry<K, V>> p : tree.preorder(root()))
   ...
   /* public int depth(Position<Entry<K, V>> p) {
    ...
    protected void print(Position<Entry<K, V>> localroot) {
        printAux(localroot);
    }

    private void printAux(Position<Entry<K, V>> localroot) {
        for (Position<Entry<K, V>> p : positions(localroot)) {
            for (int i = 0; i < depth2(p,localroot); i++)
                System.out.print(".");
            System.out.println("depth of the node : " + p.getElement());
        }
    }

    public int depth2(Position<Entry<K,V>> p,Position<Entry<K,V>> localRoot) throws IllegalArgumentException {
        if (p==localRoot)
            return 0;
        else
            return 1 + depth2(parent(p),localRoot);
    }
}